      ...
      <PageTransition>
        <div className="px-5 pt-6 pb-6">
              ...
              return (
                <div key={status.key} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    ...
                    {i < statuses.length - 1 && <div className={`w-0.5 h-12 rounded-full transition-colors duration-300 ${done ? 'bg-primary' : 'bg-muted'}`} />}
                  </div>
                  <div className="pb-6">
          ...
          <div className="flex gap-3">
            <Button size="sm" variant="outline" onClick={() => setCurrentStatus(Math.max(0, currentStatus - 1))} className="rounded-xl flex-1">
              Previous
            </Button>
            <Button size="sm" onClick={() => {
              if (currentStatus < statuses.length - 1) setCurrentStatus(currentStatus + 1);
              else navigate('/rate-service');
            }} className="rounded-xl gradient-blue text-primary-foreground flex-1 transition-opacity hover:opacity-95">
