...
import { ArrowLeft, ArrowRight, CheckCircle, User, FileText, Briefcase, Users, Shield, Clock, CreditCard, FileCheck } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background">
      <div className="gradient-hero px-5 pt-6 pb-10 rounded-b-[2rem]">
        <div className="flex items-center gap-3 mb-5">
          <button onClick={() => step > 0 ? prev() : navigate('/')} className="p-2 rounded-xl bg-primary-foreground/10"><ArrowLeft className="h-5 w-5 text-primary-foreground" /></button>
          <span className="font-bold text-primary-foreground">Join <span className="opacity-80">Cleanfit</span></span>
        </div>
        <div className="flex gap-1">
          {steps.map((_, i) => (
            <div key={i} className={`flex-1 h-1.5 rounded-full transition-colors ${i <= step ? 'bg-primary-foreground' : 'bg-primary-foreground/20'}`} />
          ))}
        </div>
      ...
      <div className="px-5 py-6">
        <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
          {step === 0 && (
            <>
              ...
              <Input placeholder="Mobile number (+44...)" className={inputClass} />
          ...
          {step === 1 && (
            <>
              ...
              <Input placeholder="Proof of address" className={inputClass} />
            </>
          )}
          {step === 2 && (
            <>
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2">
                {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => (
                  <button key={d} className="px-5 py-2.5 rounded-xl bg-muted text-muted-foreground text-sm font-medium hover:bg-primary hover:text-primary-foreground transition-all">
                    {d}
                  </button>
                ))}
              </div>
          ...
          {step === 6 && (
            <>
              ...
              <Input placeholder="Account number (8 digits)" className={inputClass} />
        ...
        <div className="flex gap-3 mt-8">
          ...
          <Button onClick={next} className="flex-1 h-12 gradient-blue text-primary-foreground rounded-2xl shadow-blue font-semibold">
